...
| 15 | **Web framework = FastAPI; orchestration = LangGraph; run state = Postgres checkpointer** | The entrypoint is an async **FastAPI** app (`POST /runs` → `run_id`, background task; `GET /runs/{id}` reads checkpointer state). The pipeline is a **LangGraph** `StateGraph` (PM→Research→Coding→Reviewer→Fixer→Merge) over one **namespaced** `WorkflowState`, persisted by the **AsyncPostgresSaver** (Postgres checkpointer for states) keyed on `thread_id`=`run_id`. Replaces the hand-rolled sync `pipeline.py`. |
...
| 27 | **PM tickets get a per-ticket elaboration pass (depth over a single compressed call)** | A comprehensive uploaded spec was yielding thin, few-line tickets because PM generated the **whole** spec (epic + tech spec + all tickets + risks) in **one** structured call — so each ticket got compressed to fit the output budget (worse on small models like `gpt-4o-mini`). Fix: after the skeleton spec + validation, PM runs a **focused second pass to elaborate tickets** (`PMAgent._elaborate_tickets`, gated by `pm_detail_tickets`, source-spec context capped by `pm_detail_context_chars`) so each ticket gets its own generation budget and comes out richly detailed. `pm_bulk_elaborate` (default True, decision #32) uses a **single bulk LLM call** for all tickets to minimize total calls (reducing ~10 calls to 2); falls back to sequential elaboration if bulk fails. `Ticket` gains structured detail fields (`implementation_notes`, `affected_files`, `api_changes`, `data_model_changes`, `out_of_scope`) that force depth and feed the build team; the pass is **best-effort + structure-preserving** (keeps the validated id/type/dependencies, falls back to the original ticket on error). Detail propagates to `brief()`/`_ticket_brief`, the file Board, and the PM-run UI. |
| 30 | **Agent↔LLM I/O is persisted and viewable per run; PM workbench has a direct listing** | Every exchange between an agent and the model (messages sent + response received, per LLM call) is captured in `agents/base.py` (`_capture_exchange` at `_invoke_structured`/`_explore`, content clipped, gated by `persist_llm_exchanges`) and persisted to the `AgentLLMExchange` table by `graph/nodes.py` (alongside `AgentRunMetric`, keyed by run/ticket/agent; the `Runner.refine_ticket` path persists its own). Viewed at `GET /ui/runs/{id}/llm` (`llm_exchanges.html`), grouped by story+agent with collapsible prompt/response. This is for **inspection, not replay** — long content is truncated with a marker; retention/cleanup is out of scope. Separately, the "PM workbench" nav lands on `GET /ui/pm-workbench`, a listing of `pm_only` runs sourced from `RunRecord` (so in-progress runs with no spec yet appear) whose rows open the workbench directly; the searchable `SpecRecord` archive remains at `/ui/pm-runs`. |
| 29 | **PM workbench = standalone PM run + two-level feedback loop; deterministic estimate repair** | PM can run **standalone** (`pm_only` on `WorkflowState`/`RunRecord`): it generates/refines a spec and **stops** at the review gate — it never auto-builds. Two conditional edges after `pm_publish`/`rfc` (`_route_after_pm_publish`/`_route_after_rfc`) keep full-pipeline runs unchanged (`pm_only=False` → original targets) while a workbench run routes to END by default, or to a one-shot **RFC** or **build-first-story** when the reviewer picks that follow-up at the gate (`{"action":"approve","next":"rfc"\|"build"}` → `PMState.next_action`). A dedicated workbench UI (`/ui/pm/{run_id}`, `pm_workbench.html`) adds a **two-level HITL feedback loop**: spec-level free-text → `Runner.regenerate_spec` (re-runs PM with the feedback threaded into the prompt, increments `regeneration_count`, clears it after use) and per-story → `Runner.refine_ticket` (re-elaborates ONE ticket via `PMAgent.refine_ticket`/`_elaborate_one`, folds it back with `aupdate_state` **without** advancing the graph; `get_run` falls back to `snapshot.next` to keep the gate visible after the in-place patch). **Single/multiple** is hardened: `build_stories` uses an `effective_mode` (workbench-build forces single) and guards a stale-`stories` leak. **Estimates** stay LLM-driven but a deterministic Python repair (`agents/estimates.py`, `pm_estimate_speedup`) parses the traditional text → days and derives the LLM-assisted days when the model's are missing/zero/inconsistent. The **CLI** now uses the Postgres checkpointer by default (`--ephemeral` for in-memory). |
| 31 | **Removed PM agent compaction** | Removed the logic that forced compact mode (truncated fields) for models with small output budgets; all models now use the standard elaboration pass for ticket depth. |
...
- **2026-06-18 — IMPLEMENTED: agent↔LLM I/O capture + direct PM workbench listing (decision #30).**
  ...
  `AgentLLMExchange` app table and viewable per run. Capture lives in `agents/base.py` at the LLM
  choke points (`_invoke_structured` → single_call/extract; `_explore` loop → per-step) into a
  per-agent `_exchanges` buffer (content clipped to 16k chars/field, gated by the new
  `persist_llm_exchanges` setting). `graph/nodes.py` resets the buffer before each `agent.run()` and
  persists it after (mirrors `_record_metric`, keyed by run/ticket/agent); the workbench refine path
  persists from `Runner.refine_ticket`. View: `GET /ui/runs/{id}/llm` (`llm_exchanges.html`) groups
  exchanges by story+agent with collapsible prompt/response panels (phase/model/token badges), linked
  from the run detail and the workbench. New `db/exchanges.py` (`record_exchanges` /
  `list_exchanges_for_run`). **Direct workbench listing:** `GET /ui/pm-workbench` lists `pm_only`
  `RunRecord`s (incl. runs still generating — sourced from RunRecord, not SpecRecord) via new
  `db/runs.py:list_workbench_runs`, rows open `/ui/pm/{id}` directly; the sidebar "PM workbench" nav
  now lands here, and the searchable spec archive stays at `/ui/pm-runs` (relabeled "PM specs",
  reachable via "All specs →"). 210 pytest green (+5), ruff + mypy --strict clean (79 source files);
  live routes smoke-tested. Design: decision #30 row in §7.

- **2026-06-18 — IMPLEMENTED: PM workbench, feedback loop, estimate & story-mode fixes.**
  PM is now runnable **standalone** — a `pm_only` run generates/refines a spec and stops at the
  review gate (never auto-builds). New conditional edges after `pm_publish`/`rfc` in `graph/builder.py`
  (`_route_after_pm_publish`/`_route_after_rfc`) preserve full-run behavior and add workbench
  stop / one-shot RFC / build-first-story paths keyed on `PMState.next_action`. New dedicated UI:
  `GET /ui/pm/{run_id}` (`pm_workbench.html` + `_pm_workbench_body.html` + `_pm_story_card.html` +
  `_pm_regenerating.html`), `GET /ui/pm/new` (reuses `run_new.html` in `pm_mode`), and HTMX endpoints
  `POST /ui/pm/{id}/regenerate` (spec-level feedback → `Runner.regenerate_spec`), `.../stories/{tid}/refine`
  (per-story → `Runner.refine_ticket` → re-render one card), and `.../rfc` / `.../build` (manual
  follow-ups). `PMState` gains `feedback`/`ticket_feedback`/`regeneration_count`/`next_action`;
  `WorkflowState`/`RunRecord` gain `pm_only` (PG backfill). `Runner` takes a `pm_agent` ref;
  `get_run` falls back to `snapshot.next` so the spec gate stays visible after an in-place refine
  patch. **Estimate fix:** new pure `agents/estimates.py` (parse + repair) runs as the last PM step
  in both the normal and compact paths (`pm_estimate_speedup`, default 6×) — fixes garbage
  LLM estimates under small `LLM_MAX_TOKENS`. **Story-mode fix:** `build_stories` `effective_mode`
  + stale-story leak guard. **CLI** defaults to the Postgres checkpointer (`--ephemeral` opt-out).
  205 pytest green (+31), ruff + mypy --strict clean (78 source files); live GET routes smoke-tested.
...
- **2026-06-18 — LangChain/LangGraph ecosystem usage doc added.**
  ...
  research sinks, P5 Instructor fallback) and a quick-reference table.
...
- **2026-06-17 — IMPLEMENTED: per-story fan-out F0–F8 (decision #26).** Shipped the full
  ...
  **Context-min (F7):** Chroma indexes line-ranged **chunks** (not whole files); search returns
  ...
  Full design + file map: `per_story_fanout_and_oversight_plan.md`.
