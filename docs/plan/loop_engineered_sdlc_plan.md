...
> Source document: `agent_architecture.md` (your multi-agent SDLC design), reconciled with the
> "loop engineering" pattern (a self-running agent harness).
> Goal: understand the requirement, reconcile the two, and lay out a way forward.
>
...
### Phase 2 — LangGraph orchestration + checkpointing + Reviewer
- Wire the graph with conditional edges and **Postgres checkpointing** (resume on crash).
- Add the **Reviewer as a separate checker agent** (distinct model/prompt from Dev).
...
| Concern | Doc | Recommendation |
|---------|-----|----------------|
| LLM | GPT-5 / Claude Opus | **Claude** (Opus for PM, Sonnet for Dev/Reviewer/Fixer) — chosen. |
| Orchestration | LangGraph | LangGraph ✓ (use built-in checkpointing). |
| Queue | Redis | **Defer.** LangGraph handles orchestration; add Redis only for true multi-worker scale. |
| State of record | PostgreSQL | Postgres for **LangGraph checkpoints**; add an `state.md`/board for human-readable external memory. |
| Code memory | pgvector | **Defer to Phase 5.** ripgrep + file reads first. |
| GitHub | API + GitPython | ✓ — PyGithub for API, GitPython for worktrees/commits. Scope the token tightly. |
| Observability | LangSmith | Add in Phase 5. |
| Parallel safety | (absent) | **Add git worktrees** — the article's key reliability primitive. |
...
| 14 | **Control plane = Django** | Platform layer is a Django project (`controlplane/` + app `house/`) persisting Client→Project→Run, with admin UI + `manage.py build`. Engine (`src/ash`) stays framework-agnostic; Django depends on it, not vice-versa. SQLite now; REST API/UI/auth/billing are Phase 5+. |
...
### 9.2 Directory layout (monorepo)
```
ash/                             # root folder
├── engine/src/ash/     # the ENGINE — framework-agnostic, written once
│   ├── agents/                  # pm, research, coding (+ reviewer, fixer later)
│   ├── gates.py                 # ApprovalGate (§7b)
│   ├── config.py                # loads projects/<name>.yaml (robust repo-root detection)
│   ├── pipeline.py              # build-team orchestration (graph wiring comes in Phase 2)
│   ├── schemas.py / state.py    # Spec/Plan/CodeChange + WorkflowState
│   ├── tools/code_intel.py      # read-only repo intelligence
│   └── connectors/              # github (issues), git_repo (worktrees), pr (gh), board (specs)
...
├── docs/                        # plan/ (authoritative) + architecture notes
├── tests/                       # pytest (engine)
├── runtime/                     # gitignored: ash.sqlite3 + plane/{board,state,worktrees}
├── manage.py · pyproject.toml · justfile · Dockerfile · docker-compose.yml · README.md
└── CLAUDE.md                    # standing self-instructions
```
The engine never imports Django; the Django control plane imports the engine. Tooling: **ruff**
(lint + format, replacing flake8/isort/black) and **pytest**.
