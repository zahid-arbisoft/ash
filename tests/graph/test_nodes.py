...
async def test_scoped_node_folds_result_into_story():
    ...
    node = make_node(OkAgent(), node_name="coding")
    update = await node(_story_state())
    assert update["stories"]["T1"].coding.note == "done"


async def test_scoped_node_captures_error_into_story():
    node = make_node(BoomAgent(), node_name="coding")
    update = await node(_story_state())
    story = update["stories"]["T1"]
    assert "kaboom" in story.coding.error
    assert story.status == "failed"
    assert story.failed_step == "coding"
