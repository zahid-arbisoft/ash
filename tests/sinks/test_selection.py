...
async def test_explicit_choice_overrides_default(session, tmp_path):
    ...
    local = await create_connector(session, name="local", kind=ConnectorKind.file, is_sink=True)
    sink = await resolve_task_sink(session, sink_id=local.id, board_dir=tmp_path)
    assert isinstance(sink, FileBoardSink)
